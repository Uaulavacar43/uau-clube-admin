...
export default function EditCouponPage() {
	...
	useEffect(() => {
		const fetchCoupon = async () => {
			try {
				setIsLoading(true);
				const response = await couponService.getCouponById(Number(id));

				// Configurar planos selecionados se existirem
				if (response && response.plans && response.plans.length > 0) {
					setSelectedPlans(response.plans.map((plan: { id: number }) => plan.id));
				}
				
				// Configurar serviços selecionados se existirem
				if (response && response.services && response.services.length > 0) {
					setSelectedServices(response.services.map((service: { id: number }) => service.id));
				}

				// Converter datas para o formato do input
				const validFrom = new Date(response.validFrom).toISOString().split('T')[0];
				const validUntil = new Date(response.validUntil).toISOString().split('T')[0];
